...
# 表選擇模型 (Node 1 的檢索層，見 utils/table_filter.py)。
#
# 一般建議是這種輔助任務要用小模型省錢省時間。這裡實測後不採用，兩個理由：
#   準確度 —— 139 題檢索召回 20B 97.1% / 120B 97.8%，差距不大，但錯的性質
#             不同：20B 漏的是問題裡明講的東西（#62「單價超過 30000 元商品」
#             漏 products、#63「哪些客戶」漏 customers），120B 漏的是真正
#             有歧義的題目。前者是不可靠，不是能力邊際差。
#   延遲   —— 在這個 NIM endpoint 上 120B 反而比較快（中位數 1.50s vs 1.72s，
