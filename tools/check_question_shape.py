...
# ⚠️ 2026-09-11：這兩份清單原本漏了整整一類。#3022「哪種錯誤最常出現？各幾次？」
# 是標準的 [B] 形狀衝突，卻沒被報出來 —— 因為「哪種」不在 _PICK、「最常」不在 _CMP。
# 同一句型換個量詞或換個副詞就繞過閘門，那不是閘門是巧合。
_PICK = ("哪個", "哪一個", "哪一", "誰", "何者", "哪家", "哪張", "哪位", "哪表",
         "哪種", "哪樣", "哪款", "哪類", "哪支", "哪本", "哪台", "哪筆", "哪批")
_CMP = ("比較多", "比較少", "比較高", "比較低", "較多", "較少", "較高", "較低",
        "最多", "最少", "最高", "最低", "最大", "最小", "最長", "最短", "最先", "最晚",
        "最常", "最久", "最快", "最慢", "最早", "最貴", "最便宜", "最熱門", "最頻繁")
...
ADJUDICATED: dict[int, str] = {
    # ---- 保留驗收集（1001~）：[E] 的解析誤判，問句其實每一欄都點名了 ----
    1075: "[E] 假陽性。問句「編號、退掉的金額，以及當初收的金額」正好對上 GT 的三欄，"
          "是 enum_count 只取最後一個小句、又把「以**及**」的『及』當成頓號拆成兩項。"
          "問句決定得了 SELECT 清單，不必收窄。",
    1081: "[E] 假陽性。問句「姓名與總額」對上 GT 的 (name, SUM)，兩項兩欄。"
          "閘門把 name 當分組鍵扣掉，但這題的分組鍵**本來就要回**（問的是「誰」），"
          "扣掉之後才對不上。同型的還有 #1110。",
    1110: "[E] 假陽性。問句「編號與種類數」對上 GT 的 (o.id, COUNT(DISTINCT ...))，"
          "兩項兩欄。與 #1081 同型：分組鍵是答案的一部分，不該被扣掉。",
